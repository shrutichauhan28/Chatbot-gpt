...
PROMPT_TEMPLATE_DOC = """
Your name is **Yugm.ai**, and you are an expert in natural language. You've been working at this company for over a decade and know every corner of its history, policies, and culture. You're not just an assistant but a friendly, approachable colleague with a wealth of experience.
...
# Initialize Milvus vectorstore and chat chain
# vectorstore = vector_database()
# conversational_chain = chain(vectorstore)

# Simulate a chat session without Streamlit
user_input = input("Ask AI: ")

basic_answer = handle_basic_questions(user_input)
if basic_answer:
    friendly_response = append_follow_up(basic_answer)
    print(friendly_response)
else:
    formatted_chat_history = [
        {"role": "user", "content": "What is the company's history?"},  # Placeholder for real chat history
    ...
    # friendly_response = append_follow_up(assistant_response)
    # print(friendly_response)

# Function to initialize chat chain with conversation memory
# vector_db=vector_db


# question_prompt = PromptTemplate(
#     input_variables=["context"],
#     template="Given the following context: {context}, generate a question."
# )

# # Create an instance of an LLM (replace with your preferred model)
# llm = OpenAI(model_name="gpt-3.5-turbo")  # This should be an instance of a language model you are using
...
#     template="Combine the following documents into a coherent response: {documents}"
# )

# # Create the combine docs chain using the same or another LLM
# combine_docs_chain = LLMChain(llm=llm, prompt=combine_prompt)
